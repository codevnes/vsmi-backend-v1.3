...
var decimalToNumber = function (decimal) {
    ...
    if ('s' in decimal && 'e' in decimal && 'd' in decimal && Array.isArray(decimal.d)) {
        ...
        var digits = decimal.d.join('');
        // Reconstruct the number
        if (exponent <= 0) {
            // Value is less than 1
            return sign * Number("0.".concat('0'.repeat(Math.abs(exponent))).concat(digits));
        }
        else if (exponent >= digits.length) {
            // Value is an integer with trailing zeros
            return sign * Number("".concat(digits).concat('0'.repeat(exponent - digits.length)));
        }
        else {
            // Value has digits on both sides of decimal point
            return sign * Number("".concat(digits.substring(0, exponent), ".").concat(digits.substring(exponent)));
        }
    }
    return decimal;
};
 ...
 */
var bigIntSerializer = function (data) {
    ...
    if (Array.isArray(data)) {
        return data.map(function (item) { return (0, exports.bigIntSerializer)(item); });
    }
    // Handle objects
    if (typeof data === 'object') {
        ...
        for (var key in data) {
            if (Object.prototype.hasOwnProperty.call(data, key)) {
                result[key] = (0, exports.bigIntSerializer)(data[key]);
            }
        }
        return result;
    }
