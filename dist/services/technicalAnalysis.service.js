...
class TechnicalAnalysisService {
    async getAll({ page, limit, search }) {
        ...
        const [technicalAnalyses, total] = await Promise.all([
            app_1.prisma.technicalAnalysis.findMany({
                where,
                skip,
                take: limit,
                orderBy: { symbol: 'asc' },
            ...
            app_1.prisma.technicalAnalysis.count({ where }),
        ]);
        return {
    ...
    async getBySymbol(symbol) {
        return app_1.prisma.technicalAnalysis.findUnique({
            where: {
                symbol,
            },
            include: {
    ...
    async update(symbol, data) {
        return app_1.prisma.technicalAnalysis.update({
            where: { symbol },
            data,
            include: {
                stock: true,
    ...
    async upsert(symbol, data) {
        return app_1.prisma.technicalAnalysis.upsert({
            where: { symbol },
            create: {
                symbol,
                stock: {
    ...
    async delete(symbol) {
        return app_1.prisma.technicalAnalysis.delete({
            where: { symbol },
        });
    }
}
