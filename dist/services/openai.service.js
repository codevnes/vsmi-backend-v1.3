...
class OpenAIService {
    ...
    constructor() {
        ...
        try {
            this.promptTemplate = fs_1.default.readFileSync(TECHNICAL_RECOMMENDATION_PROMPT_PATH, 'utf8');
        }
        catch (error) {
     ...
     */
    async processTechnicalRecommendation(symbol, date) {
        try {
            // Get the last 20 technical recommendations for the symbol
            const recommendations = await app_1.prisma.technicalRecommendation.findMany({
                where: { symbol },
                orderBy: { date: 'desc' },
                take: 20,
            });
            ...
            const sortedData = [...recommendations].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
            ...
            const savedAnalysis = await app_1.prisma.chatGptAnalysis.upsert({
                where: {
                    symbol_analysisDate: {
                        symbol,
     ...
     */
    formatTechnicalRecommendationsForGPT(recommendations) {
